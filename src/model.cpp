...
void Config::from_yalm(YALMData &yalm, int context) {
	...
	n_experts_active = yalm.metadata.contains("n_experts_active")
	...
	qkv_clip =
	    yalm.metadata.contains("qkv_clip") ? std::stof(yalm.metadata.at("qkv_clip").get<std::string>()) : FLT_MAX;
	...
	if (dtype == "fp32") {
		...
		weight_dtype = DType::F8E5M2; // Using F8E5M2 format as specified in convert.py
	} else {
...
void *check_tensor(const Tensor *tensor, DType weight_dtype, std::array<int, 4> shape) {
	...
	if (tensor->dtype != weight_dtype || tensor->shape != shape) {
		...
		std::cerr << fmt::format("expected: dtype={}, shape=[{},{},{},{}]", dtype_to_string(weight_dtype), shape[0],
		                         shape[1], shape[2], shape[3])
		          << std::endl;
		std::cerr << fmt::format("got: dtype={}, shape=[{},{},{},{}]", dtype_to_string(tensor->dtype), tensor->shape[0],
...
Block::Block(int layer_i, const std::shared_ptr<Config> config, const Tensor *rms_att_weight,
             const Tensor *rms_ffn_weight, const Tensor *wq, const Tensor *wk, const Tensor *wv, const Tensor *wo,
             const Tensor *w1, const Tensor *w2, const Tensor *w3, const Tensor *moegate) {
	_layer_i = layer_i;
	_config = config;
	...
	_rms_att_weight = static_cast<float *>(check_tensor(rms_att_weight, DType::F32, {config->dim, 0, 0, 0}));
	_rms_ffn_weight = static_cast<float *>(check_tensor(rms_ffn_weight, DType::F32, {config->dim, 0, 0, 0}));

	_wq = check_tensor(wq, config->weight_dtype, {config->n_heads * config->head_dim, config->dim, 0, 0});
	_wk = check_tensor(wk, config->weight_dtype, {config->n_kv_heads * config->head_dim, config->dim, 0, 0});
	_wv = check_tensor(wv, config->weight_dtype, {config->n_kv_heads * config->head_dim, config->dim, 0, 0});
	_wo = check_tensor(wo, config->weight_dtype, {config->dim, config->n_heads * config->head_dim, 0, 0});

	if (config->n_experts > 0) {
		_moegate = check_tensor(moegate, config->weight_dtype, {config->n_experts, config->dim, 0, 0});
		_w1 = check_tensor(w1, config->weight_dtype, {config->n_experts, config->hidden_dim, config->dim, 0});
		_w2 = check_tensor(w2, config->weight_dtype, {config->n_experts, config->dim, config->hidden_dim, 0});
		_w3 = check_tensor(w3, config->weight_dtype, {config->n_experts, config->hidden_dim, config->dim, 0});
	} else {
		_w1 = check_tensor(w1, config->weight_dtype, {config->hidden_dim, config->dim, 0, 0});
		_w2 = check_tensor(w2, config->weight_dtype, {config->dim, config->hidden_dim, 0, 0});
		_w3 = check_tensor(w3, config->weight_dtype, {config->hidden_dim, config->dim, 0, 0});
	}
...
void Block::cuda() {
	...
	_key_cache = static_cast<f16_t *>(
	    upload_cuda(_key_cache, _config->max_seq_len * _config->n_kv_heads * _config->head_dim * sizeof(f16_t)));
	_value_cache = static_cast<f16_t *>(
	    upload_cuda(_value_cache, _config->max_seq_len * _config->n_kv_heads * _config->head_dim * sizeof(f16_t)));
}

void Block::block(InferenceState &s, // inference state
	...
	if (_device == Device::CUDA) {
		...
		case DType::F8E5M2: {
			_block_cuda<f16_t>(s, pos, kv_sink, kv_pos, kv_len); // Temporarily use f16_t path for F8E5M2
			break;
		}
		default: {
		...
		case DType::F8E5M2: {
#if defined(__AVX2__) && defined(__F16C__)
			_block_cpu<f16_t>(s, pos, kv_sink, kv_pos, kv_len); // Temporarily use f16_t path for F8E5M2
#else
			assert(false && "float8 not supported on this platform");
#endif
			break;
		}
		default: {
...
InferenceState::InferenceState(const std::shared_ptr<Config> config) : _config(config) {
	assert(config);
...
void InferenceState::cuda() {
	...
	if (_moe_weights != nullptr) {
		...
		_active_experts_weights =
		    static_cast<float *>(upload_cuda(_active_experts_weights, _config->n_experts_active * sizeof(float)));
	}
}
...
Model::Model(YALMData &yalm, int context) {
	...
	token_embedding_table = check_tensor(get_tensor(yalm, "model.embed.weight"), config->weight_dtype,
	                                     {config->vocab_size, config->dim, 0, 0});

	for (int i = 0; i < config->n_layers; ++i) {
		blocks.emplace_back(std::make_shared<Block>(
		    i, config, get_tensor(yalm, fmt::format("model.layers.{}.attn.norm.weight", i)),
	...
	rms_final_weight =
	    static_cast<float *>(check_tensor(get_tensor(yalm, "model.norm.weight"), DType::F32, {config->dim, 0, 0, 0}));
	bool tie_word_embeddings = yalm.tensors.count("model.output.weight") == 0;
	if (tie_word_embeddings) {
		wcls = token_embedding_table;
	} else {
		wcls = check_tensor(get_tensor(yalm, "model.output.weight"), config->weight_dtype,
		                    {config->vocab_size, config->dim, 0, 0});
	}
}
