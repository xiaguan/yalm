...
struct InferenceState {
	...
	float *xb(int head) const {
		return _xb + _config->head_dim * head;
	}
	// TODO: do we need xb2?
	float *xb2() const {
		return _xb2;
	}
	float *xb2(int head) const {
		return _xb2 + _config->head_dim * head;
	}
	float *hb() const {
		return _hb;
	}
	...
	float *q(int head) const {
		return _q + _config->head_dim * head;
	}
	float *k() const {
		return _k;
	}
	...
	float *att(int head) const {
		return _att + _config->max_seq_len * head;
	}
	// mixture of experts
	float *moe_weights() const {
		return _moe_weights;
	}
	float *active_experts_weights() const {
		return _active_experts_weights;
	}
	int *active_experts() const {
		return _active_experts;
	}
	// LM head
	float *logits() const {
		return _logits;
...
private:
	...
	float *_active_experts_weights =
	    nullptr;                    // (n_active_experts,) - buffer for weights of top K experts (active experts)
...
/* Transformer Block */
struct Block {
	Block(int layer_i, const std::shared_ptr<Config> config, const Tensor *rms_att_weight, const Tensor *rms_ffn_weight,
	      const Tensor *wq, const Tensor *wk, const Tensor *wv, const Tensor *wo, const Tensor *w1, const Tensor *w2,
	      const Tensor *w3, const Tensor *moegate);
	~Block();

	float *rms_att_weight() const {
		return _rms_att_weight;
	}
	float *rms_ffn_weight() const {
		return _rms_ffn_weight;
	}
	template <typename T>
	T *wq() const {
		return static_cast<T *>(_wq);
	}
	template <typename T>
	T *wk() const {
		return static_cast<T *>(_wk);
	}
	template <typename T>
	T *wv() const {
		return static_cast<T *>(_wv);
	}
	template <typename T>
	T *wo() const {
		return static_cast<T *>(_wo);
	}
	template <typename T>
	T *w1() const {
		return static_cast<T *>(_w1);
	}
	template <typename T>
	T *w2() const {
		return static_cast<T *>(_w2);
	}
	template <typename T>
	T *w3() const {
		return static_cast<T *>(_w3);
	}
	template <typename T>
	T *moegate() const {
		return static_cast<T *>(_moegate);
	}
	f16_t *key_cache() const {
		return _key_cache;
	}
	f16_t *value_cache() const {
		return _value_cache;
	}
...
private:
	template <typename T>
	void _block_cpu(InferenceState &s, // inference state
	                int pos,           // index of the current token in the sequence
	                int kv_sink,       // number of sink tokens currently in the KV cache
	...
	template <typename T>
	void _block_cuda(InferenceState &s, // inference state
	                 int pos,           // index of the current token in the sequence
	                 int kv_sink,       // number of sink tokens currently in the KV cache
	                 int kv_pos, // index of the current token in the kv cache, must be in [0..kv_len) since kv cache is
	                             // a ring buffer
