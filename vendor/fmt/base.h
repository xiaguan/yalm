...
public:
	...
	basic_string_view(const Char *s)
	    : data_(s), size_(detail::const_check(std::is_same<Char, char>::value && !detail::is_constant_evaluated(false))
...
public:
	...
	basic_format_args(const detail::format_arg_store<Context, NUM_ARGS, NUM_NAMED_ARGS, DESC> &store)
...
struct format_specs {
	...
	constexpr format_specs()
	    : width(0), precision(-1), type(presentation_type::none), align(align::none), sign(sign::none), upper(false),
	      alt(false), localized(false) {
	}
};
...
public:
	explicit FMT_CONSTEXPR format_string_checker(basic_string_view<Char> fmt)
	    : types_ {mapped_type_constant<Args, buffered_context<Char>>::value...}, context_(fmt, num_args, types_),
